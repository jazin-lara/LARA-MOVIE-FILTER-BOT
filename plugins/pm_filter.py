    ...
    if gl == False:
        ...
        if k == False:
            ...
            if files:
                ...
                if NO_RESULTS_MSG:
                    await bot.send_message(chat_id=-1002764274382, text=(script.NORSLTS.format(reqstr.id, reqstr.mention, movie)))
...
async def auto_filter(client, name, msg, reply_msg, ai_search, spoll=False):
    ...
    if imdb:
        ...
        if not settings["button"]:
            ...
            btn.append(
                [InlineKeyboardButton("Hello", url="https://t.me/MrTG_Coder")]
            )
    else:
        if settings["button"]:
